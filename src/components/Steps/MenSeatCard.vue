  ...
  >
    ...
    <v-row v-if="!editable">
      <v-col
        ...
        md="12"
      >
        <p> {{form.firstName }} {{form.lastName}}, {{form.streetAddress}} </p>
        <v-spacer></v-spacer>
        <v-btn
          color="primary"
          @click="editable = true"
...
export default {
  ...
  computed: {
    ...
    icon () {
      if (this.requestings.length === this.requesting.length) return 'fa close-box'
  ...
  watch: {
    requestings () {
      this.additionalData.rh1 = this.requestings.find( request => request === 'Rosh Hashanah I') ? true : false
      this.additionalData.rh2 = this.requestings.find( request => request === 'Rosh Hashanah II') ? true : false
      this.additionalData.yk = this.requestings.find( request => request === 'Yom Kippur') ? true : false
    },
  },
  created () {
    console.log("asdf")
    const owner = this.groupRequest.owner
    if(owner.gender === 'Male' && this.counter === 1){
      this.form = {
        ...this.form,
        ...owner
      }
